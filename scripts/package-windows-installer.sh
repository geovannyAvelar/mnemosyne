#!/usr/bin/env bash
# Run inside an MSYS2 MINGW64 shell, after scripts/package-windows.sh has
# already staged a working copy of the app (Mnemosyne.exe + Qt + MinGW/MSYS2
# DLLs) — this script doesn't restage anything, it just wraps that existing
# staging directory into a proper installer via NSIS (Start Menu shortcut,
# optional Desktop shortcut, and a real uninstaller in Add/Remove Programs).
set -euo pipefail

BUILD_DIR="${1:-build}"
STAGE_DIR="$(pwd)/$BUILD_DIR/windows-stage"
OUT_FILE="$(pwd)/$BUILD_DIR/Mnemosyne-Setup.exe"
