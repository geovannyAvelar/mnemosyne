...
- **PDF** — rendered via Poppler, with zoom, table of contents navigation, and text search. Password-protected files are supported: opening one prompts for its password (retrying on a wrong guess) rather than just failing. A "Two-Page" toggle shows facing pages side by side, like an open book, instead of one page per row; the choice is remembered for the next PDF you open.
...
- **MOBI/AZW/AZW3** — rendered via [libmobi](https://github.com/bfabiszewski/libmobi) (LGPLv3), with the same chapter/table-of-contents/zoom/dark-mode/highlight feature set as EPUB, including embedded images (both classic Mobipocket `recindex` and KF8 `kindle:embed` references). **DRM-protected files are refused outright** — Mnemosyne never attempts to decrypt anything; it's a reader for files you already have unencrypted access to, not a DRM-removal tool.
- **CBZ comics** — a zip of page images, reusing the same archive-reading code EPUB uses (no new dependency). Page navigation and zoom like PDF; no table of contents, search, or highlights, since there's no text layer to drive them. Optional two-page spread mode, with a right-to-left (manga) reading order.
...
- **Typography** (EPUB/MOBI/Markdown/TXT) — the toolbar's "Aa" button offers a font family, line spacing, and margin width, on top of zoom. A reading preference applied across the whole app (like Dark Mode), not saved per book.
- **Library / tabs** — open multiple books at once in tabs, with a persistent Library tab and recent-files list. Organize your library with shelves (named collections a book can belong to) and free-form tags, both filterable from the Library view; right-click (desktop) or the "⋯" button (mobile) on a book to assign it.
- **Reading stats** — time spent and pages read today, your current/longest day streak, average pages/day, and all-time totals across your whole library. Desktop: the sidebar's Stats tab. Mobile: the 📊 button on the Library screen.
- **Highlights and notes**, synced across devices — highlight a passage in a color of your choosing, optionally attach a note to it, and browse every note in the sidebar's Notes tab. Exportable via File > Export Notes, as a Markdown document or as Anki flashcards, for the open book or your whole library at once. For PDFs specifically, "as Annotated PDF..." bakes highlights and freehand drawings into a new copy of the PDF itself as real PDF annotations (highlights, ink strokes, and each highlight's note as a clickable annotation popup) — visible and printable in any PDF reader, not just Mnemosyne.
