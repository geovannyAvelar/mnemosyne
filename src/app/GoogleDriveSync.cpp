...
{
    const QString dir =
        QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + QStringLiteral("/GoogleDriveSync");
    QDir().mkpath(dir);
    return dir;
}
...
// Drive file ID for this device's own <deviceId>.jsonl, once known — avoids
// a lookup-by-name round trip on every append after the first one this run.
QString &ownFileId()
{
    static QString id;
    return id;
...
// modifiedTime last seen per remote file name, so latestFromOtherDevices
// doesn't re-download files that haven't changed since the last check.
    ...
    GoogleAuth::withAccessToken([fileId, content](const QString &token) {
        ...
        httpClient().patch(url, {{QStringLiteral("Authorization"), authHeader(token)}}, content,
                            QStringLiteral("text/plain"), [](const IHttpClient::Response &) {
                                ...
                                // position won't reach other devices until the next append.
                            });
    });
}
...
void createFile(const QString &name, const QByteArray &content)
{
    GoogleAuth::withAccessToken([name, content](const QString &token) {
        ...
        const QByteArray boundary = "mnemosyne_sync_boundary";
        QByteArray body;
        body += "--" + boundary + "\r\n";
        ...
        httpClient().post(
            ...
            [name](const IHttpClient::Response &response) {
                ...
                if (!id.isEmpty() && name == DeviceIdentity::id() + QStringLiteral(".jsonl")) {
                    ownFileId() = id;
                }
            });
    });
...
void uploadOwnFile(const QByteArray &content)
{
    ...
    const QString name = DeviceIdentity::id() + QStringLiteral(".jsonl");
    GoogleAuth::withAccessToken([name, content](const QString &token) {
        if (token.isEmpty()) {
            return;
        }
...
void appendEntry(const QString &bookHash, const QString &title, int position, qreal zoom)
{
    ...
    ProgressSyncLog::appendEntryToDirectory(dir, DeviceIdentity::id(), DeviceIdentity::name(), bookHash, title,
                                             position, zoom);

    QFile file(QDir(dir).filePath(DeviceIdentity::id() + QStringLiteral(".jsonl")));
    ...
    GoogleAuth::withAccessToken([bookHash, excludeDeviceId, callback](const QString &token) {
        ...
        httpClient().get(
            ...
            [bookHash, excludeDeviceId, callback, token](const IHttpClient::Response &response) {
                ...
                const QString excludeFileName = excludeDeviceId + QStringLiteral(".jsonl");
                const QString dir = stagingDirectory();
                ...
                for (const QJsonValue &value : files) {
                    ...
                    if (name == excludeFileName || !name.endsWith(QStringLiteral(".jsonl"))) {
                        continue;
                    }
                ...
                if (toDownload.isEmpty()) {
                    callback(ProgressSyncLog::latestFromDirectory(dir, bookHash, excludeDeviceId));
                    return;
                }
